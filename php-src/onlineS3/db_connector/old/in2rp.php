...
function in2rp($conn) {
	...
	// if ($var_res->num_rows > 0) {
		// while($row = $var_res->fetch_assoc()) {
			...
			// $sql_insert = "INSERT INTO variable (var_id, description) VALUES ('$var_id', '$description')";
			
			// $conn->query($sql_insert);
		// }
	// }
	
	$imp_sel = "SELECT in_data.description, reg.reg_id as reg_id, in_data.var_id, reg.reg_id FROM in_data JOIN region reg ON in_data.region = reg.label";
	...
	for ($i=0; $i<count($year_array); $i++) {
		...
		$sql_base = "INSERT INTO rp_data (var_id, reg_id, year_id, value)
						...
						JOIN region reg ON ind.region = reg.label AND rt.reg_id = reg.reg_id WHERE ind.$year > 0";
